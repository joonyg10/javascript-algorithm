...
- 셀 수있는 수량의 순서있는 열거 또는 어떤 순서를 따르는 요소들의 모음을 튜플(tuple)이라고 합니다. n개의 요소를 가진 튜플을 n-튜플(n-tuple)이라고 하며, 다음과 같이 표현할 수 있습니다.
  (a1, a2, a3, ..., an)

- 특정 튜플을 표현하는 집합이 담긴 문자열이 인자로 주어질 때, 해당 문자열이 표현하는 튜플을 반환하시오
...
1. s는 "{{2},{2,1},{2,1,3},{2,1,3,4}}" 와 같이 주어 지는데 {} 기준으로 나눌 수 있는 방법을 모색해야한다.

- 이 문제에서는 /[^{\}]+(?=})/ 의 정규표현식을 사용한다.
- [2], [2,1], [2,1,3] ... 의 형태로 분리가 가능!

2. 1)에서 분리된 문자에 대해 길이순으로 오름차순 정렬한다.
3. 맨 앞에서 부터 각 배열에 대해 비교를 시행
   (1) 원소가 hashMap에 있는 경우 -> continue
   (2) hashMap에 없다면, set을 해준다. (value는 임의의 값이어도 좋다.)
   (3) (2)에 대해 answer: 튜플 에 parseInt한 값을 추가한다.
